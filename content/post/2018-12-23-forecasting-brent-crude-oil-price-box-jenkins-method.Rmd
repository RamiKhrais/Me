...
<p style='text-align: justify;'>In this porject, the Box-Jenkins Method has been utilized to model the prices of brent crude oil for the period from May-1987 to Nov-2018. Five models have been simulated upon different AR and MA orders and three models of them were selected based on the value of AIC. The three models have then been diagnosed using the residuals analysis. The seasonal model ARIMA (1,1,0)(0,0,2), which was chosen by auto.arima function, proved to be the best-fit model. The three models were used to forecast a new set of data and model ARIMA (1,1,0) showed the best performance with lowest value of RMSE. Though our model proved to be little bit accurate in forecasting new data, it cannot be considered as a reliable one under any condition. The forecasting of crude oil prices has always been a hard task. The prices of crurde are so sensitive to a wide range of complicated and multi-faceted factors including, among others, the volume of inventories, the weather, the speculation and the political tensions. Constructing models that can take into account these factors(though some of them are difficult to be qunatified) might provide much better forecasts than these which depend solely on historical data. </p>
