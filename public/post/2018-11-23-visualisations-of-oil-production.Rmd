...
```{r warning=FALSE, message=FALSE}
colnames(bp)[1] <- "Country" #Changing the column name of “Thousand Barrels”

bp_clean1 <- bp[1:(length(bp)-3)] #Removing the last three redundant columns

bp_clean2 <- na.omit(bp_clean1) #Removing na values form the data

bp_clean3 <- gather(bp_clean2, Year, Production, -Country, na.rm = TRUE) #Rearranging the data

bp_clean3$Production <- as.integer(bp_clean3$Production) #Changing the prodution into integer

bp_clean3$Production[is.na(bp_clean3$Production)] <- 0 #Replacing n/a values with zeros
...
ggplot(top_ten, aes(x = reorder(Country, Production), y = Production)) + 
  ...
  geom_text(aes(y= Production, x = seq(15,1), label = paste0(round(Production),"")), nudge_y = 700, color="black", size = 5) + 
  theme_minimal() +
  theme(axis.title.x = element_blank(),
