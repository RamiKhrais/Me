...
ggplot(market_snapshot, aes(x = reorder(Carrier, flights), y = flights)) + 
  ...
  geom_text(aes(y= flights, x = seq(15,1), label = paste0(round(flights),"")), nudge_y = 4500, color="black", size = 5) + 
  theme_minimal() +
...
<p style='text-align: justify;'>Everthing looks good so far. Let's try now to get some spatial insights by aaking: What is the most active departure airport for each carrier in weekend?</p>
...
<p style='text-align: justify;'>**What is the most active airline routes in the Weekend?**</p>
...
ggplot(Cancelled_Carr, aes(x=reorder(Carrier, cancelledflights), y=cancelledflights)) + 
  ...
  theme(plot.title = element_text(size = 14, face = "bold", colour = "black", margin = margin(0,0,30,0)))
```
